...
final class Loader {
	...
	public function view($route, $data = array()) {
		...
		$query = $db->query("SELECT value FROM " . DB_PREFIX . "setting WHERE store_id = '" . (int)$store_id . "' AND `key` = 'config_disable_cart'");
		
		$settingValue = 0;
		if ($query->num_rows) {
			$settingValue = $query->row['value'];
		};
		
		$data['global_disable_cart'] = $settingValue;
