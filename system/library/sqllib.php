...
class SqlStatement {
    ...
    public function limit($limit, $offset = 0) {
        ...
        $this->_limit = array('offset' => (int)$offset, 'limit' => (int)$limit);
        return $this;
    }
    /**
     * HAVING statement
    ...
    public function insertInto($tbl, $values, $columns = null) {
        $this->_insert = array(
            'into' => self::quoteName(self::$DB_PREFIX . $tbl),
            'columns' => $columns,
            'values' => $values
            );
        $this->_mode = 'insert';
    ...
    public function buildInsertQuery()
    {
        $sql = 'INSERT' . (($this->_ignore) ? ' IGNORE' : '') 
             . ' INTO ' . self::quoteName($this->_insert['into']);
        ...
        if (!empty($this->_set)) {
            ...
            $sql .= ' (' . implode(',', array_map(array(__CLASS__, 'quoteName'), $columns)) . ')';
            ...
            if ($this->_insert['values'] instanceof SqlStatement || is_string($this->_insert['values'])) {
                ...
                foreach ($this->_insert['values'] as $vals) {
                    ...
                    $arr[] = '(' . implode(',', array_map(array(__CLASS__, 'quote'), $vals)) . ')';
                }
    ...
    public function update($table)
    {
        $this->_mode = 'update';
        if (is_array($table)) {
            $tblName = (is_string(current($table))) ? self::quoteName(self::$DB_PREFIX . current($table)) : current($table);
            $this->_update = array(key($table) => $tblName);
        } else {
            $this->_update = self::quoteName(self::$DB_PREFIX . $table);
        }
    ...
    public function set($set){
        ...
        foreach ($set as $setItem) {
            ...
            if (count($params)) {
                foreach ($params as $param) {
                    $condition = str_replace('?', self::quote($param), $condition);
                }
            }
        ...
        if (count($this->_join)) {
            foreach ($this->_join as $alias => $join) {
                ...
                if ($join['table'] instanceof SqlStatement) {
                    ...
                    $sql .= self::quoteName($join['table']);
                }
                if (!is_numeric($alias)) {
                    $sql .= ' AS ' . self::quoteName($alias);
                }
                $sql .= ' ON (' . $join['on'] . ')';
            }
        }
    ...
    public static function quote($i)
    {
