...
class ControllerExtensionModulePavmegamenu extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')  && !empty($this->request->post) ) {
			if (!$this->user->hasPermission('modify', 'extension/module/'.$this->moduleName)) {
				...
				if( isset($id) && $this->request->post['save_mode']=='save-edit'){
                    $this->redirect($this->url->link('extension/module/pavmegamenu', 'id='.$id.'&user_token=' . $this->session->data['user_token'].$store_param, 'SSL'));
				}	else {
					$this->redirect($this->url->link('extension/module/pavmegamenu', 'user_token=' . $this->session->data['user_token'].$store_param, 'SSL'));
				}
				$this->redirect($this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token']. '&type=module', 'SSL'));
			}
		}
		...
		$this->mdata['button_save_new'] = $this->language->get('button_save_new');
		$this->mdata['button_save_edit'] = $this->language->get('button_save_edit');
		$this->mdata['button_add_module'] = $this->language->get('button_add_module');
		$this->mdata['button_remove'] = $this->language->get('button_remove');
		
		$this->mdata['text_process_request'] = $this->language->get('text_process_request');
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('extension/extension/pavmegamenu', $url.'&user_token=' . $this->session->data['user_token'].'&type=module', 'SSL');
		}
		...
		$this->mdata['breadcrumbs'][] = array(
   		...
   		$this->mdata['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/pavmegamenu', 'user_token=' . $this->session->data['user_token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->mdata['action'] 			= $this->url->link('extension/module/pavmegamenu', 'user_token=' . $this->session->data['user_token'].$store_param, 'SSL');
		$this->mdata['actionGetTree'] 	= $this->url->link('extension/module/pavmegamenu/gettree', 'user_token=' . $this->session->data['user_token'].$store_param, 'SSL');
		$this->mdata['actionDel'] 		= $this->url->link('extension/module/pavmegamenu/delete', 'user_token=' . $this->session->data['user_token'].$store_param, 'SSL');
		$this->mdata['actionGetInfo'] 	= $this->url->link('extension/module/pavmegamenu/info', 'user_token=' . $this->session->data['user_token'].$store_param, 'SSL');
		$this->mdata['updateTree'] 		= $this->url->link('extension/module/pavmegamenu/update', 'root=1'.$store_param.'&user_token=' . $this->session->data['user_token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/module/pavmegamenu/pavmegamenu', $this->mdata));
	}
	...
	public function delete(){
		...
		$this->redirect($this->url->link('extension/module/pavmegamenu', 'user_token=' . $this->session->data['user_token'].$store, 'SSL'));
	}
	...
	public function liveedit(){
		...
		$this->mdata['filemagement_uri'] = $this->url->link('common/filemanager', '1=1&token=' . $this->session->data['user_token'], 'SSL'); 
		...
		$this->mdata['action_backlink'] = $this->url->link('extension/module/pavmegamenu', 'root=1'.$store_param.'&user_token=' . $this->session->data['user_token'], 'SSL');
		$this->mdata['action_widget'] 	= HTTP_CATALOG.'index.php?route=extension/module/pavmegamenu/renderwidget';
		$this->mdata['action_addwidget']= $this->url->link('extension/module/pavmegamenu/addwidget', 'user_token=' . $this->session->data['user_token'].$store_param, 'SSL'); 

		$this->mdata['action_wform'] 	= $this->url->link('extension/module/pavmegamenu/widgetform', 'user_token=' . $this->session->data['user_token'].$store_param, 'SSL'); 

		$this->mdata['ajxgenmenu'] 	   	= $this->url->link('extension/module/pavmegamenu/ajxgenmenu', 'root=1'.$store_param.'&user_token=' . $this->session->data['user_token'], 'SSL'); 
		$this->mdata['ajxmenuinfo'] 	= $this->url->link('extension/module/pavmegamenu/ajxmenuinfo', 'root=1'.$store_param.'&user_token=' . $this->session->data['user_token'], 'SSL'); 
	...
	public function addwidget(){
		...
		if( isset($this->request->post['widget']) && isset($this->request->post['params']) ){
			...
			$this->redirect( $this->url->link('module/pavmegamenu/addwidget', 'done=1&id='.$row['id'].'&wtype='.$row['type'].'&user_token=' . $this->session->data['user_token'], 'SSL') ); 
		}
		...
		$this->mdata['action'] = $this->url->link('module/pavmegamenu/addwidget', 'user_token=' . $this->session->data['user_token'], 'SSL'); 
	...
	public function delwidget(){
		...
		$this->redirect($this->url->link('module/pavmegamenu', 'user_token=' . $this->session->data['user_token'], 'SSL'));
	}
