...
class ControllerExtensionBlogBlogSetting extends Controller { 
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/blog/blog_setting', 'user_token=' . $this->session->data['user_token'], true));
		}
		...
		$url = '';
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/blog/blog_setting', 'user_token=' . $this->session->data['user_token'] . $url, true)
   		);
		...
		$data['action'] = $this->url->link('extension/blog/blog_setting', 'user_token=' . $this->session->data['user_token'], true);
		...
		if (isset($this->request->post['blogsetting_background'])) {
			$data['blogsetting_background'] = $this->request->post['blogsetting_background'];
		} elseif ($this->config->get('blogsetting_background')) {
			$data['blogsetting_background'] = $this->config->get('blogsetting_background');
		} else {
			$data['blogsetting_background'] = '';
		}
		...
		if (isset($this->request->post['blogsetting_background']) && is_file(DIR_IMAGE . $this->request->post['blogsetting_background'])) {
			$data['back_thumb'] = $this->model_tool_image->resize($this->request->post['blogsetting_background'], 100, 100);
		} elseif ($this->config->get('blogsetting_background')) {
			$data['back_thumb'] = $this->model_tool_image->resize($this->config->get('blogsetting_background'), 100, 100);
		} else {
			$data['back_thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		if (isset($this->request->post['blogsetting_rel_prod_width'])) {
			...
			$data['blogsetting_rel_prod_width'] = 266;
		}
		...
		if (isset($this->request->post['blogsetting_rel_prod_height'])) {
			...
			$data['blogsetting_rel_prod_height'] = 266;
		}
	...
	public function setSettings() {
		...
		$blog_data = array(
			...
			'blogsetting_rel_thumbs_w' => '408',
			'blogsetting_rel_thumbs_h' => '204',
			'blogsetting_rel_prod_height' => '266',
			'blogsetting_rel_prod_width' => '266',
			'blogsetting_blogs_per_page' => '5',
			'blogsetting_thumbs_w' => '848',
			'blogsetting_thumbs_h' => '424',
			'blogsetting_date_added' => '1',
			'blogsetting_page_view' => '1',
			'blogsetting_author' => '1',
