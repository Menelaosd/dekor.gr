...
class ControllerCatalogProduct extends Controller {
	...
	public function pricelist() {
		...
		$this->getListFast(false);
	}	
	
	public function pricelistApokrifa() {
		$this->load->language('catalog/product');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_noimage'])) {
				$url .= '&filter_noimage=' . $this->request->get['filter_noimage'];
			}

			if (isset($this->request->get['filter_xml'])) {
				$url .= '&filter_xml=' . $this->request->get['filter_xml'];
			}

			if (isset($this->request->get['filter_isskroutz'])) {
				$url .= '&filter_isskroutz=' . $this->request->get['filter_isskroutz'];
			}
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_noimage'])) {
			$filter_noimage = $this->request->get['filter_noimage'];
		} else {
			$filter_noimage = '';
		}
		...
		if (isset($this->request->get['filter_isskroutz'])) {
			$filter_isskroutz = $this->request->get['filter_isskroutz'];
		} else {
			$filter_isskroutz = '';
		}
		...
		$filter_data = array(
			...
			'filter_noimage'   => $filter_noimage,
			'filter_manufacturer' => $filter_manufacturer,
			'filter_xml'   	  => $filter_xml,
			'filter_isskroutz'=> $filter_isskroutz,
			'sort'            => $sort,
			'order'           => $order,
		...
		foreach ($results as $result) {
			...
			$product_seo_urls = $this->model_catalog_product->getProductSeoUrlsOrder($result['product_id']);
			$product_seo_urls[0]['store'] = 'DEKOR';
			$product_seo_urls[0]['store_id'] = '0';
			$product_seo_urls[2]['store'] = 'APOKRIFA';
			$product_seo_urls[2]['store_id'] = '2';
			...
			$data['products'][] = array(
				...
				'special'    => $special,
				'product_seo_urls'    => $product_seo_urls,
				'xml_id'	 => $result['xml_id'], 
				'sort_order'	 => $result['sort_order'],
				'quantity'   => $result['quantity'],
				'disablecart'   => $result['disablecart'],
				'storeonly'   => $result['storeonly'],
		...
		$data['filter_noimage'] = $filter_noimage;
		$data['filter_xml'] = $filter_xml;
		$data['filter_isskroutz'] = $filter_isskroutz;
	...
	protected function getListFast($apokrifa = false) {
		if (isset($this->request->get['filter_name'])) {
			$filter_name = $this->request->get['filter_name'];
		} else {
			$filter_name = '';
		...
		if($apokrifa) {
			$results = $this->model_catalog_product->getProductsApokrifa($filter_data);
		} else {
			$results = $this->model_catalog_product->getProductsDekor($filter_data);
		}
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'weight'   =>  number_format($result['weight'],2),
				'package'   => $result['package'],
				'price_unit' => (isset($result['package']) && $result['package'] > 0) ? ($result['price'] / $result['package']) : $result['price'],
				'status'     => $result['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
				'edit'       => $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $result['product_id'] . $url, true)
			);
		}
		...
		if($apokrifa) {
			$this->response->setOutput($this->load->view('catalog/product_pricelist_apokrifa', $data));
		} else {
			$this->response->setOutput($this->load->view('catalog/product_pricelist', $data));
		}
	}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['disablecart'])) {
			$data['disablecart'] = $this->request->post['disablecart'];
		} elseif (!empty($product_info)) {
			$data['disablecart'] = $product_info['disablecart'];
		} else {
			$data['disablecart'] = true;
		}
		
		if (isset($this->request->post['storeonly'])) {
			$data['storeonly'] = $this->request->post['storeonly'];
		} elseif (!empty($product_info)) {
		...
		// DETAILS STUFF  -- START
		if (isset($this->request->post['details_data'])) {
			$data['details_data'] = $this->request->post['details_data'];
		} elseif (isset($this->request->get['product_id'])) {
			$data['details_data'] = $this->model_catalog_product->getProductDetails($this->request->get['product_id']);
		} else {
			$data['details_data'] = array();
		}		
		// DETAILS STUFF  -- END
