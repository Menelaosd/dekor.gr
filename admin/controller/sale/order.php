...
class ControllerSaleOrder extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->get['filter_order_paidshipping'])) {
			$url .= '&filter_order_paidshipping=' . $this->request->get['filter_order_paidshipping'];
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_order_paidshipping'])) {
			$filter_order_paidshipping = $this->request->get['filter_order_paidshipping'];
		} else {
			$filter_order_paidshipping = '';
		}
		...
		$filter_data = array(
			...
			'filter_order_paidshipping'   => $filter_order_paidshipping,
			'sort'                   => $sort,
			'order'                  => $order,
		...
		$data['filter_order_paidshipping'] = $filter_order_paidshipping;
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'product_id'       => $product['product_id'],
					'package'       => $this->model_sale_order->getProductPackage($product['product_id']),
					'name'    	 	   => $product['name'],
					'model'    		   => $product['model'],
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
					...
					'package_price'    => $this->currency->format( ( ($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0)) / $this->model_sale_order->getProductPackage($product['product_id'])) + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
					
					'pricenotax'    => $this->currency->format($product['price'], $order_info['currency_code'], $order_info['currency_value']),
					'simpletax'     => $this->currency->format(($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0) - $product['price']), $order_info['currency_code'], $order_info['currency_value']),
				...
				foreach ($products as $product) {
					...
					if ($product_info) {
						...
						$product_data[] = array(
							...
							'package' => $this->model_sale_order->getProductPackage($product_info['product_id']),
