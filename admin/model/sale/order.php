...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id,o.telephone,o.erp_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS order_status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";

		if (!empty($data['filter_order_paidshipping'])) {
			$sql .= " LEFT JOIN  ". DB_PREFIX . "order_total ot ON(ot.order_id = o.order_id)";
		}
		...
		if(!empty($data['filter_date_from']) && !empty($data['filter_date_to']) ) {
			$sql .= " AND ( DATE(o.date_added) >= DATE('" . $this->db->escape($data['filter_date_from']) . "') AND DATE(o.date_added) <= DATE('" . $this->db->escape($data['filter_date_to']) . "') )";
		} else {
		...
		if (!empty($data['filter_order_paidshipping'])) {
			$sql .= " AND ot.code = 'shipping' AND ot.value > 0 ";
		}
	...
	public function getProductPackage($product_id) {
		$query = $this->db->query("SELECT package FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
		
		$product = $query->row;
		$product_package = $product['package'];
		
		if($product_package <= 1) {
			$product_package = 1;
		}
