...
class ControllerExtensionOnepagecheckoutSocial extends Controller {
	public function index() {
		...
		$data['flogin'] = $this->url->link('extension/onepagecheckout/social/fblogin','','SSL');
		$data['glogin'] = $this->url->link('extension/onepagecheckout/social/glogin','','SSL');
		
		
		return $this->load->view('extension/onepagecheckout/social', $data);
		
	}
	
	public function glogin(){
		session_start();
		require_once(DIR_SYSTEM. "library/onepagechekout/google/autoload.php");
	...
	public function fblogin(){
		session_start();
		require_once(DIR_SYSTEM."library/onepagechekout/Facebook/autoload.php");
	...
	public function facebook(){
		...
		try {
		  $accessToken = $helper->getAccessToken($homeurl);
		...
		if($fbuser){
			if(isset($fbuser['email'])){
					$displayname = explode(' ',$fbuser['name']);
					if (isset($displayname[0])) {
						$data['first_name'] = $displayname[0];
					}else{
						$data['first_name'] = '';
					}
			
					if (isset($displayname[1])) {
						$data['last_name'] = $displayname[1];
					} else {
						$data['last_name'] = '';
					}
				...
				if(!empty($customer_id)){
					...
					$this->response->redirect($this->url->link('extension/onepagecheckout/social', '&token=' . $token));
				}else{
