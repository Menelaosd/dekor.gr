<?php
class ControllerExtensionOnepagecheckoutCart extends Controller {
	...
	public function index() {
		...
		if(!empty($onepagecheckout_manage['shopping_cart']['shopping_cart_status'])) {
			...
			if(!empty($onepagecheckout_manage['shopping_cart'][$coupon_status])) {
				$data['coupon_module'] = $this->load->controller('extension/onepagecheckout/coupon');	
			}else{
				$data['coupon_module'] = '';
			}
			
			if(!empty($onepagecheckout_manage['shopping_cart'][$reward_status])) {
				$data['reward_module'] = $this->load->controller('extension/onepagecheckout/reward');
			}else{
				$data['reward_module'] = '';
			}
			
			if(!empty($onepagecheckout_manage['shopping_cart'][$voucher_status])) {
				$data['voucher_module'] = $this->load->controller('extension/onepagecheckout/voucher');
			}else{
				$data['voucher_module'] = '';
			}
			...
			foreach ($products as $product) {
				...
				if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
					...
					$price_tax = $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], false), $this->session->data['currency']);
				} else {
					$price = false;
					$price_tax = false;
				}
				...
				if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
					...
					$total_tax = $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], false) * $product['quantity'], $this->session->data['currency']);
				} else {
					$total = false;
					$total_tax = false;
				}
			...
			$this->response->setOutput($this->load->view('extension/onepagecheckout/cart', $data));
