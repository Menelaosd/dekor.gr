<?php
session_start();
class ControllerExtensionOnepagecheckoutPaymentMethod extends Controller {
	public function index() {
		...
		if (isset($this->session->data['payment_address'])){
			...
			foreach ($results as $result) {
				if ($this->config->get('payment_'.$result['code'] . '_status') && !empty($onepagecheckout_manage['payment_method'][$result['code']]['status'])) {
			...
			$_SESSION['temp_payment_methods'] = $method_data;
		}
		...
		}elseif(!empty($this->session->data['temp_payment_method']['code'])){
			$data['code'] = $this->session->data['temp_payment_method']['code'];
		} else {
		...
		$this->response->setOutput($this->load->view('extension/onepagecheckout/payment_method', $data));
		
	}
	
	public function savepayment(){
		if(!isset($this->session->data['payment_methods'])){
			$this->session->data['payment_methods'] = $_SESSION['temp_payment_methods'];
		}
		...
		if (!$json) {
			...
			$this->session->data['temp_payment_method'] = $this->session->data['payment_methods'][$this->request->post['payment_method']];
			$json['success'] = true;
		}
