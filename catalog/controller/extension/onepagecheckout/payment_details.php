...
class ControllerExtensionOnepagecheckoutPaymentdetails extends Controller {
	...
	public function index(){
		...
		foreach($fields as $key => $feild){
			...
			if($status){
				$data['feilds'][$key]=array(
					...
					'required'		=> $required,
					'key'					=> $key,
					'label'				=> $onepagecheckout_manage['payment_details_language'][$key]['label'][$this->config->get('config_language_id')],
		...
		$data['entry_heading'] = $onepagecheckout_manage['payment_details_language']['heading_title'][$this->config->get('config_language_id')];
