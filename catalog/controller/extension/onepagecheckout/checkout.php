<?php
class ControllerExtensionOnepagecheckoutCheckout extends Controller {
	private $error=array();
	private $ssl = 'SSL';
	private $json=array();
	...
	public function index(){
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$data['isTelephone'] = $this->customer->getTelephone();
			$data['design_status'] = $this->config->get('onepagecheckout_design_status');
			$data['tabbgcolor'] = $this->config->get('onepagecheckout_tab_background_color');
			$data['tabtextcolor'] = $this->config->get('onepagecheckout_tab_text_color');
			$data['activetabbgcolor'] = $this->config->get('onepagecheckout_tab_active_background_color');
			$data['activetabcolor'] = $this->config->get('onepagecheckout_tab_active_text_color');
			$data['blockheadingbgcolor'] = $this->config->get('onepagecheckout_block_heading_bgcolor');
			$data['blockheadingcolor'] = $this->config->get('onepagecheckout_block_heading_color');
			$data['buttonbgcolor'] = $this->config->get('onepagecheckout_button_bg_color');
			$data['buttoncolor'] = $this->config->get('onepagecheckout_button_color');
			$data['customcss'] = $this->config->get('onepagecheckout_custom_css');
			
			$data['entry_telephone'] = (isset($onepagecheckout_manage['register']['telephone']['label'][$this->config->get('config_language_id')]) ? $onepagecheckout_manage['register']['telephone']['label'][$this->config->get('config_language_id')] : 'Telephone');
			...
			$data['social_login']='';
			$fbstatus = (!empty($onepagecheckout_manage['social_login']['facebook_login_status']) ? $onepagecheckout_manage['social_login']['facebook_login_status'] : '');
			
			$gstatus = (!empty($onepagecheckout_manage['social_login']['google_login_status']) ? $onepagecheckout_manage['social_login']['google_login_status'] : '');
			
			if($fbstatus || $gstatus){
			 $data['social_login'] = $this->load->controller('extension/onepagecheckout/social');
			}
			...
			$data['login'] = $this->load->controller('extension/onepagecheckout/login');
			$data['personal_details'] = $this->load->controller('extension/onepagecheckout/personal_details');
			$data['payment_details'] = $this->load->controller('extension/onepagecheckout/payment_details');
			$data['delivery_details'] = $this->load->controller('extension/onepagecheckout/shipping_address');
			$data['delivery_date'] = $this->load->controller('extension/onepagecheckout/delivery_date');
		
			$this->response->setOutput($this->load->view('extension/onepagecheckout/checkout', $data));
		}else{
			$data['heading_title'] = $heading_title;
	...
	public function country(){
		...
		if ($country_info){
			...
			$json = array(
				...
				'zone'              => $this->model_extension_onepagecheckout_customer->getZonesByCountryId($this->request->get['country_id']),
