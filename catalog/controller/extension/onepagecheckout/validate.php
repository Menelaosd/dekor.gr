...
class ControllerextensionOnepagecheckoutValidate extends Controller {
	...
	private function setPersonalRequiredFeilds(){
		...
		$fields['customer_group_id'] = array(
			'sort_order' => -999,
			'label' => '',
			'show' => 2
		);
	...
	public function validateForm(){
			...
			if(!empty($this->request->post['account_type']) && ($this->request->post['account_type'] == 'register' || $this->request->post['account_type'] == 'guest')){
				foreach($this->requireds as $key => $required){
					...
					if($key == 'customer_group_id') {
						if ($this->request->post['personal_details']['customer_group_id'] == '') {
							$json['error']['personal_details'][$key] = 'Επιλέξτε είδος Παραστατικού';
						}
					}
		...
		//	echo '</pre>';
			...
			if($this->request->post['personal_details']['customer_group_id'] == 3 && isset($this->request->post['personal_details']['custom_field']['account'][3])) {
				// afm validations
				$validator = new Ibericode\Vat\Validator();
				$afm_validate = $validator->validateVatNumber('EL'.$this->request->post['personal_details']['custom_field']['account'][3]);
				if (!$afm_validate) {
					$json['error']['personal_details']['custom_field3'] = 'ΤΟ Α.Φ.Μ Δεν είναι έγκυρο ή δεν μπορεί να τιμολογηθεί';
					
					$json['error']['warning'] = 'ΤΟ Α.Φ.Μ Δεν είναι έγκυρο ή δεν μπορεί να τιμολογηθεί';
				}
			}
			...
			if(!empty($require_comment_status['personaldetails']['terms'])){
					...
					if ($information_info && !isset($this->request->post['personal_details']['agree']) && $this->request->post['account_type'] !='guest') {
						$json['error']['warning'] = sprintf($this->language->get('error_agree'), $information_info['title']);
					}
			}
			...
			if (isset($this->request->post['shipping_method']) && $this->request->post['shipping_method'] == 'boxnow.boxnow') {
				if (!isset($this->session->data['boxnow_locker_id']) || !$this->session->data['boxnow_locker_id']) {
					$json['error']['shipping_method']['warning'] = $this->language->get('error_boxnow');
				}
			}				
		...
		if(isset($json['error']) && !isset($json['error']['warning'])){
			$json['error']['warning'] = (isset($onepagecheckout_manage['general']['warning'][$this->config->get('config_language_id')]) ? $onepagecheckout_manage['general']['warning'][$this->config->get('config_language_id')] : 'Warning: Please check the form carefully for errors!');
		}
	...
	private function save($data,$customer_group_id){
		...
		if($data['account_type']=='register'){
			...
			$customer_id = $this->model_extension_onepagecheckout_customer->addCustomer($data['personal_details']);
			...
			if ($customer_group_info && !$customer_group_info['approval']) {
				...
				if ($this->cart->hasShipping()){
					if (!empty($data['personal_details']['shipping_address'])){
						...
						$this->load->model('extension/onepagecheckout/customer');
						$address_id = $this->model_extension_onepagecheckout_customer->addAddress($data['delivery_details']);
						$this->session->data['shipping_address'] = $this->model_account_address->getAddress($address_id);
					}
				}
	...
	public function updatetelephone(){
		$this->language->load('extension/onepagecheckout/checkout');
		if(!empty($this->request->post['telephone'])){
			...
			$json['success'] = $this->language->get('text_success_telephone');
		}else{
			$onepagecheckout_manage = $this->config->get('onepagecheckout_manage');
			$json['error']  = (isset($onepagecheckout_manage['register']['error'][$this->config->get('config_language_id')]) ? $onepagecheckout_manage['register']['error'][$this->config->get('config_language_id')] : 'error telephone');
		}
