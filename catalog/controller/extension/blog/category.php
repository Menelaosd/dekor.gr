	...
	if (isset($this->request->get['blogpath'])) {
		...
		if ($blog_category_info) {
			...
			if ($blog_category_info['image']) {
				$data['thumb'] = $this->model_tool_image->cropsize($blog_category_info['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_height'));
			} else {
				$data['thumb'] = '';
			}
			...
			$data['blogs'][] = array(
				...
				'href'  => $this->url->link('extension/blog/blog', 'blogpath=' . $this->request->get['blogpath'] . '&blog_id=' . $result['blog_id'])
			...
			if ((float)VERSION >= 2.2) {
				$this->response->setOutput($this->load->view('extension/blog/blog_category', $data));
			} else {
