...
class ControllerExtensionBlogHome extends Controller {
	...
	public function index() { 
		...
		if ($this->config->get('blogsetting_background')) {
			$data['thumb'] = $this->model_tool_image->cropsize($this->config->get('blogsetting_background'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_height'));
		} else {
			$data['thumb'] = '';
		}		
		...
		// EXCLUDE LAST BLOG FROM RESULTS AND STORE IT IN DATA['LAST_BLOG']
		if (!empty($results)) {
			//$last_blog = array_shift($results);
		}
		
		$last_blog = null;

		$data['last_blog'] = array(
			'count_read' => $last_blog['count_read'],
			'blog_id' => $last_blog['blog_id'],
			'title'      => $last_blog['title'],
			'short_description'      => html_entity_decode($last_blog['short_description'], ENT_QUOTES, 'UTF-8'),
			'date_added_full' => date($this->language->get('date_format_short'), strtotime($last_blog['date_added'])),
			'author' => $last_blog['author'],
			'image' => $this->model_tool_image->cropsize($last_blog['image'], 763, 424),
			'href' => $this->url->link('extension/blog/blog', 'blog_id=' . $last_blog['blog_id'])
			);
    	...
    	foreach ($results as $result) {
			...
			'image' => $this->model_tool_image->cropsize($result['image'], $img_width, $img_height),
		...
		if($data['store_id'] == 2) {
			$data['heading_title'] = 'Apokrifa Blog';
		} else {
			$data['heading_title'] = 'Dekor Blog';
		}
		...
		if($data['store_id'] == 2) {
			$data['description'] = 'Καλώς ήρθες σε έναν χώρο αφιερωμένο στο απόκρυφο, την αρχαία σοφία και τη μυστική γνώση. Εδώ θα ανακαλύψεις τελετουργίες, βότανα, σύμβολα, λιβάνια και παλιές πρακτικές που γεφυρώνουν τον ορατό με τον αόρατο κόσμο. Αν σε έλκει το μυστήριο, η δύναμη της πρόθεσης και η σοφία της παράδοσης, τότε βρίσκεσαι στο σωστό μέρος.';
		} else {
			$data['description'] = 'Στο blog μας θα βρείτε έμπνευση και ιδέες για κάθε εποχή και περίσταση με πρωταγωνιστή το κερί — από ανοιξιάτικα citronella μέχρι χριστουγεννιάτικα στολίδια και πασχαλινές λαμπάδες. Ανακαλύψτε τη μαγεία του χειροποίητου κεριού σε κάθε γιορτή και σημαντική στιγμή της ζωής σας.';
		}
		...
		$this->response->setOutput($this->load->view('extension/blog/blog_home', $data));
	}
}