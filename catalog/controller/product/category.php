...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if (isset($this->request->get['sort'])) {
			...
			// $sort = 'p.sort_order';
			$sort = 'p.sort_order ASC, p2c.sort_order_ppc';
		}
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				if ($result['image']) {
					...
					$result['thumb'] = $this->model_tool_image->resize('no_image.png', $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_height'));
				}
			...
			// $data['display_products'] = (($category_info['parent_id'] == 0) && (!empty($data['categories']))) ? 0 : 1;
			$data['display_products'] = ((!empty($data['categories'])) ? 0 : 1);
			...
			foreach ($results as $result) {
				...
				if($result['package'] && $result['package'] > 1) {
				...
				if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
					...
					if($result['package'] && $result['package'] > 1) {
						$package_price = $this->currency->format($this->tax->calculate($result['price'] / $result['package'], $result['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
					}
				...
				if ((float)$result['special']) {
					...
					if($result['package'] && $result['package'] > 1) {
						$package_special = $this->currency->format($this->tax->calculate($result['special'] / $result['package'], $result['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
					}
				} else {
					$special = false;
					$special_net = false;
				...
				if ($this->config->get('config_tax')) {
					...
					if($result['package'] && $result['package'] > 1) {
						$package_tax = $this->currency->format( (float) ($result['special'] ? $result['special'] : $result['price']) / $result['package'], $this->session->data['currency']);
					}					
				} else {
					$tax = false;
				}
