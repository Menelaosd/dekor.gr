...
function MobileUtilities(){
	if (window.innerWidth < 769) {
		$('#shopping_cart').appendTo('#ho_mmenu');
		$('.product_helpers').after($('#column-left'));
		$('#input-sort').appendTo($('#category_sort'));

		$('#category_filters').click(function(e){
			e.preventDefault();
...
$(document).ready(function() {
	...
	$('.hasMegamenu').mouseenter(function(){
		...
		var highest_col = 0;
		$('.level1.mega-cols .col-level1').height('');
		$('.level1.mega-cols .col-level1').each(function(){
			$this = $(this);
			if ($this.outerHeight() > highest_col) {
				highest_col = $this.outerHeight();
			}
		});
		$('.level1.mega-cols .col-level1').outerHeight(highest_col);
	}).mouseleave(function(){
