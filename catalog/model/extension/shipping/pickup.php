...
class ModelExtensionShippingPickup extends Model {
	function getQuote($address) {
		...
		$allowed_prefixes = ['10', '11', '12', '13', '14', '15', '16', '181', '182', '183', '184', '185'];
		$postcode = preg_replace('/\s+/', '', $address['postcode']); // remove any spaces
		$status = false;
		
		foreach ($allowed_prefixes as $prefix) {
			if (strpos($postcode, $prefix) === 0) {
				if (!$this->config->get('shipping_pickup_geo_zone_id') || $query->num_rows) {
					$status = true;
				}
				break;
			}
