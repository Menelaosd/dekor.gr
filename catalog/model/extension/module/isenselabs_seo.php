    ...
    public function MissingPageWorker($route, $store_id = 0) {
        ...
        if ($redirect_table_check) {
            ...
            if (!$route_check->num_rows) {
                $wildcards = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_404_redirects` WHERE `route_from` LIKE '%*%' AND `store_id` = '" . (int)$store_id . "'");

                foreach ($wildcards->rows as $pattern) {
                    if (fnmatch($pattern['route_from'], $route)) {
                        $route_check = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_404_redirects` WHERE `route_from` = '" . $this->db->escape($pattern['route_from']) . "' AND `store_id` = '" . (int)$store_id . "' LIMIT 1");
                    }
                }
            }
