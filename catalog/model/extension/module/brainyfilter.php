...
final class ModelExtensionModuleBrainyFilter extends Model 
{
    ...
    private function _prepareFilterQuery($filterByPrice = true)
    {
        ...
        if (!empty($this->aggregate)) {
            ...
            foreach ($this->aggregate as $group => $vals) {
                $vals = array_map(array('SqlStatement', 'quote'), $vals);
    ...
    public function prepareQueryForCategory()
    {
        ...
        switch ($this->sortBy) 
        {
			...
			case 'p.sort_order ASC, p2c.sort_order_ppc' : {
                $sql->select(array('p2c.sort_order_ppc'))->order(array("p2c.sort_order_ppc {$this->order}"))
                    ->innerJoin(array('p2c' => 'product_to_category'), 'p2c.product_id = p.product_id');
                break;
            }
            case 'p.date_added' : {
