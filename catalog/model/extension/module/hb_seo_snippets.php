...
class ModelExtensionModuleHbSeoSnippets extends Model {	
		...
		if ($this->config->get('hb_snippets_prod_enable')) {
			...
			if ($review_query->rows) {
				...
				foreach ($reviews as $rev) {
					$review_data[] = '{"@type": "Review","author": "'.$rev['author'].'", "datePublished": "'.date('Y-m-d', strtotime($rev['date_added'])).'", "description": "'.$rev['text'].'", "reviewRating": {"@type": "Rating","bestRating": "5","ratingValue": "'.$rev['rating'].'","worstRating": "1"}}';
				}
	...
	public function category_social($category_info){
		...
		if ($this->config->get('hb_snippets_og_enable')){
			...
			$this->document->setOpengraph('og:site_name', $this->config->get('config_name'));
			$this->document->setOpengraph('og:url', $this->url->link('product/category', 'category_id=' . $category_info['category_id']));
