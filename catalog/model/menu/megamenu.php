...
class ModelMenuMegamenu extends Model {		
		...
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				...
				if( $this->hasChild($menu['megamenu_id']) || $menu['type_submenu'] == 'html'){			
					...
					$output .= '<li class="parent dropdown '.(($menu['is_group'] == 1) ? 'hasMegamenu': 'noMegamenu').' '.$menu['menu_class'].'" '.$this->renderAttrs($menu).'>';				

					$output .= '<a class="dropdown-toggle" data-toggle="dropdown1" href="'.$this->getLink( $menu ).'">';
	...
	public function genMegaMenuByConfig( $parentId, $level,$menu  ){
		...
		$output .= '<a href="'.$this->getLink( $menu ).'" class="dropdown-toggle" data-toggle="dropdown1">';
			...
			$output .= '<div class="row">';
				foreach( $row->cols as $col ){
					$colclass = isset($col->colclass)?$col->colclass:'';
					 $output .= '<div class="mega-col col-level'.$level.' col-xs-12 col-sm-12 col-md-'.$col->colwidth.' '.$colclass.'" '.$this->getColumnDataConfig( $col ).'> <div class="mega-col-inner">';
	...
	public function genTree( $parentId, $level,$parent, $store_id = 0){
		...
		$attrw = '';
		$class = $parent['is_group']?"dropdown-mega level".$level : "dropdown-menu";
		...
		if( $parent['type_submenu'] == 'html' ){
			...
			if( $parent['colums'] > 1  ){
				...
				if( !empty($parent['megaconfig']->rows) ) {
						...
						foreach( $rows as $rowcols ){
							...
							foreach( $rowcols as $key => $col ) {
								...
								if( isset($col->type) && $col->type == 'menu' && isset($cols[$key]) ){
									$scol = '<div class="mega-col  col-level'.$level.' col-xs-12 col-sm-12 col-md-'.$col->colwidth.'" data-type="menu" '.$this->getColumnDataConfig( $col ).'><div class="mega-col-inner">';
									$scol .= '<ul>';
									...
									$scol = '<div class="mega-col  col-level'.$level.' col-xs-12 col-sm-12 col-md-'.$col->colwidth.'"  '.$this->getColumnDataConfig( $col ).'><div class="mega-col-inner">';
					...
					$output = '<div class="'.$class.' mega-cols  col-level'.$level.' cols'.$parent['colums'].'" '.$attrw.' ><div class="dropdown-menu-inner"><div class="row">';
					$cols   = array_chunk( $data, ceil(count($data)/$parent['colums'])  );
					...
					foreach( $cols as $i =>  $menus ){
						...
						$output .='<div class="mega-col  col-level'.$level.' '.$oSpans[$i+1].' col-'.($i+1).'" data-type="menu"><div class="mega-col-inner"><ul>';
			...
			///	echo '<pre>' .print_r( $parent, 1 );
				if( !empty($parent['megaconfig']->rows) ) {
						...
						foreach( $rows as $rowcols ){
							...
							foreach( $rowcols as $col ) {
								...
								if( isset($col->type) && $col->type == 'menu' ){
									...
									$scol = '<div class="mega-col col-level'.$level.'col-xs-12 col-sm-12 col-md-'.$colwidth.'" data-type="menu" '.$this->getColumnDataConfig( $col ).'><div class="mega-col-inner">';
									$scol .= '<ul>';
									foreach( $data as $menu ){
										$scol .= $this->renderMenuContent( $menu , $level+1 );
									}	
					...
					$output = '<div class="'.$class.' level'.$level.'" '.$attrw.' ><div class="dropdown-menu-inner">';
					$row = '<div class="row"><div class="col-sm-12 col-level'.$level.' mega-col" data-colwidth="12" data-type="menu" ><div class="mega-col-inner"><ul>';
					foreach( $data as $menu ){
	...
	public function mMenu( $parent=1 , $edit=false, $params, $store_id = 0){
		...
		if ($childs) {
			...
			foreach($childs as $child ){
				...
				if ($child['parent_id'] == 1) {
					$output .= '<li>';
					$output .= '<a href="'.$this->getLink($child).'">';
					$output .= $child['title'];
					$output .= '</a>';

					$second_output = '';
					foreach($childs as $second ){
						if ($second['parent_id'] == $child['megamenu_id']) {
							$second_output .= '<li>';
							$second_output .= '<a href="'.$this->getLink($second).'">';
							$second_output .= $second['title'];
							$second_output .= '</a>';

							$third_output = '';
							foreach($childs as $third ){
								if ($third['parent_id'] == $second['megamenu_id']) {
									$third_output .= '<li>';
									$third_output .= '<a href="'.$this->getLink($third).'">';
									$third_output .= $third['title'];
									$third_output .= '</a>';
								}
							}
							if ($third_output) {
